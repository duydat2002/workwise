...
const labelSearch = computed(() => {
  return props.projects
    .flatMap((p) => p.createdTaskLabels)
    .filter((l) =>
      l.name.toLowerCase().includes(searchLabel.value.toLowerCase())
    );
});

const createdTaskLabels = computed(() => {
  return props.projects.flatMap((p) => p.createdTaskLabels);
});
...
const members = computed(() => {
  ...
  return props.projects
    .flatMap((p) => p.members)
    .filter((m) => {
      if (memberIds.includes(m.user.id)) {
        return false;
      } else {
...
watch(
  ...
  () => {
    ...
    const projectsTemp = cloneDeep(props.projects);
    projectsTemp?.forEach((project) => {
      project.taskGroups.forEach((g) => {
        g.tasks.forEach((t) => {
...
<template>
  <div class="flex flex-wrap items-center justify-between px-5 gap-3 pb-3">
              ...
              <div class="flex flex-col">
                ...
                <div class="flex flex-wrap gap-1">
                  <div class="flex items-center flex-wrap gap-1">
                    <template
                      v-for="label in createdTaskLabels"
                      :key="label.id"
                    >
                      <div
